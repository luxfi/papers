...
X & Exchange & AVM & LUX & UTXO set, atomic swap state, NFT ownership \\
...
$$\text{Quasar} = \text{ZKP}\left(\text{BLS}_{C} \parallel \text{Ringtail}_{Q} \parallel \text{ML-DSA}_{K}\right)$$

\begin{enumerate}
\item \textbf{C-Chain}: Validators produce BLS12-381 aggregate signatures (fast path).
\item \textbf{Q-Chain}: Validators produce Ringtail LWE threshold signatures (PQ path).
...
A block is final when the Z-Chain receipt attests to valid signatures from all three paths with $\geq 67\%$ validator agreement on each. Under the assumption that at least one of BLS12-381, Ringtail-LWE, or ML-DSA remains secure, finality cannot be forged.
\end{theorem}
...
\item \textbf{Q-Chain}: NTT coefficient tables + Ringtail aggregation buffer
