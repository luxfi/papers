...
\title{\textbf{Lux Network: A 12-Chain Architecture\\for Orthogonal, Composable Blockchain Infrastructure}}

\author{Zach Kelling\thanks{zach@lux.network}\\
...
\begin{abstract}
We present the complete multi-chain architecture of Lux Network: 11 consensus chains plus one read-only query layer, each with dedicated global state, transaction types, and ZapDB storage. The primary network (P, X, C) provides the consensus foundation. Nine specialized chains (A, B, D, I, K, M, Q, Z, and G) each handle exactly one concern with no functional overlap. Cross-chain composition occurs via Warp messages at the consensus level. The D-Chain DEX serves as a universal price oracle enabling gas payment in any listed asset across all chains. Quasar consensus coordinates C+Q+M+Z for post-quantum triple-signature receipts. This paper defines each chain's state model, transaction types, fee structure, and inter-chain composition protocol.
\end{abstract}
...
Q & Quantum & Post-quantum & LUX & Corona key state, NTT tables, PQ sig aggregation \\
...
\section{Subnet Integration}

Subnets (Zoo, Pars, Hanzo, custom L1s) participate in the architecture as follows:

\begin{itemize}
\item Each subnet has its own native coin (ZOO, PARS, AI, etc.)
\item Subnet validators may be a subset of Lux primary validators or independent
\item Cross-subnet communication uses Warp messages via B-Chain
\item Subnet coins are tradeable on D-Chain for gas flexibility
\item D-Chain oracle provides the price feed for cross-subnet gas payment
\end{itemize}

\section{Quasar Consensus}

The Quasar consensus protocol spans four chains for post-quantum block finality:

$$\text{Quasar} = \text{ZKP}\left(\text{BLS}_{C} \parallel \text{Corona}_{Q} \parallel \text{ML-DSA}_{K}\right)$$

\begin{enumerate}
\item \textbf{C-Chain}: Validators produce BLS12-381 aggregate signatures (fast path).
\item \textbf{Q-Chain}: Validators produce Corona LWE threshold signatures (PQ path).
...
\item \textbf{Z-Chain}: Wraps all three verifications in a STARK receipt. The receipt is the proof of finality.
\end{enumerate}

\begin{theorem}[Quasar Finality]
A block is final when the Z-Chain receipt attests to valid signatures from all three paths with $\geq 67\%$ validator agreement on each. Under the assumption that at least one of BLS12-381, Corona-LWE, or ML-DSA remains secure, finality cannot be forged.
\end{theorem}
...
\begin{itemize}
\item \textbf{P-Chain}: Validator records, delegation state, subnet membership (UTXO model)
...
\item \textbf{Q-Chain}: NTT coefficient tables + Corona aggregation buffer
