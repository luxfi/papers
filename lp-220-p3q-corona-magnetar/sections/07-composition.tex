...
PQ-fast and PQ-strict is the price of cross-family Ring-LWE
...
function validateBatch(
    ...
    if (mode >= PQ_STRICT) {
        // P3Q Corona leg (Ring-LWE family independence)
...
sequencer$\to$Z-Chain submission pattern; LP-220 specifies the
...
format; LP-220 sections~\ref{sec:pulsar-kind}--\ref{sec:magnetar-kind}
...
who wants per-kind verifier mechanics reads LP-220; a reader who
...
LP-0120 & Quasar mainnet defaults -- owns slots \texttt{0x012204},
  ...
  (the non-rollup-batch use case); LP-218/LP-220 P3Q family
...
Corona eprint 2024/1113 & Ring-LWE threshold reference -- Corona-kind verifier conformant \\
\bottomrule
\end{tabular}
\caption{LP-220 composition references.}
...
\item \textbf{GPU dispatch boundary closure.} Install
  ...
  Blackwell verify projections (sections~\ref{sec:corona-kind}
  and~\ref{sec:magnetar-kind}) become measurements.
...
\item \textbf{Blackwell-discounted Corona gas.} Operators with
  ...
  than CPU-class; LP-220 does not codify this. A future
