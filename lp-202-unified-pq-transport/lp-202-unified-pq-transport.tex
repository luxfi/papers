...
\code{9631} Avalanche staking-P2P, and \code{9632} ZAP-native RPC ---
...
This is a \emph{transport} change only. The Snowman++/Avalanche message
set and the BFT decision logic are unchanged byte-for-byte; consensus
messages ride QUIC streams instead of a shared TCP byte stream. The
proven Avalanche TCP staking path (\code{9631}) remains the negotiated
...
\begin{itemize}[nosep]
  ...
  \item \code{:9631} --- a single long-lived TCP stream per peer carrying
        \emph{all} Avalanche P2P and consensus traffic
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Extend Avalanche, do not break it.} TCP staking on
...
\begin{itemize}[nosep]
  ...
  \item A hybrid post-quantum handshake: \code{X25519MLKEM768} for the
        ...
        proof that preserves the 20-byte Avalanche \code{NodeID} and
  ...
  \item A byte-preserving map of the full Avalanche/Simplex message set
...
\begin{description}[nosep,leftmargin=1.6cm]
  ...
  \item[Class~C (consensus-P2P)] The Avalanche/Simplex message set
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{A 1-byte stream-class prefix} is written as the first
        ...
        Class~C stays byte-identical to the Avalanche message encoding,
...
The Avalanche/Simplex message set (\code{proto/p2p}) is carried
...
\bottomrule
\end{tabular}
\caption{Avalanche/Simplex messages to QUIC stream classes. Semantics
...
The proven Avalanche TCP staking listener on \code{9631/tcp} is the
...
Avalanche/Simplex protocol is safe (no two honest nodes finalise
...
classical Avalanche TCP path remains the negotiated fallback until a
