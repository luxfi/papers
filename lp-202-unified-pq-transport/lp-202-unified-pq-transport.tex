...
\begin{itemize}[nosep]
  \item \code{:9630} --- HTTP/JSON-RPC for the \code{/v1/*} client API
  ...
  \item \code{:9632} --- an opt-in ZAP-native RPC listener
        (\code{ZAP\_RPC\_LISTEN}) that re-serves the same \code{/v1/*}
...
\emph{same} \code{/v1/*} handler chain as the HTTP listener in the
...
\begin{description}[nosep,leftmargin=1.6cm]
  \item[Class~R (RPC)] The \code{/v1/*} client API. \emph{One
