...
\paragraph{Compact bundle pulses.} Each group emits one Pulsar pulse (size on the order of the canonical Ringtail signature). $G$ groups produce $G$ pulses; aggregating across groups via Merkle is straightforward. Total bundle-cert overhead is $\mathcal{O}(G)$, not $\mathcal{O}(N)$.
...
The Go canonical (\cite{ringtailgo}) parameterizes the group construction; the consensus engine (\cite{consensusgo}, \texttt{protocol/quasar/grouped\_threshold.go}) consumes group-level signatures and assembles the quorum-of-groups Horizon Pulse. The 258-test \texttt{protocol/quasar} suite includes \texttt{grouped\_threshold\_test.go} exercising the assembly path; production $G > 1$ deployments are gated on the open-research questions listed above.
