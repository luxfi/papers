...
The Go canonical (\cite{pulsar}) parameterizes the group construction; the consensus engine (\cite{consensusgo}, \texttt{protocol/quasar/grouped\_threshold.go}) consumes group-level signatures and assembles the quorum-of-groups Horizon Pulse. The 258-test \texttt{protocol/quasar} suite includes \texttt{grouped\_threshold\_test.go} exercising the assembly path; production $G > 1$ deployments are gated on the open-research questions listed above.
