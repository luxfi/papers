...
\paragraph{Compact bundle pulses.} Each group emits one Pulsar pulse (size on the order of the canonical Corona signature). $G$ groups produce $G$ pulses; aggregating across groups via Merkle is straightforward. Total bundle-cert overhead is $\mathcal{O}(G)$, not $\mathcal{O}(N)$.
