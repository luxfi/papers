...
invalidate the proof artifacts. A future wire version (a new version
byte under the same ABI) may carry a renamed magic such as
\texttt{"SFR1"} once the proof artifacts are refreshed and
...
can register an additional wire version (post-Plonky3-upstream-rotation
or a recursive variant) under the same ABI without re-allocating the slot.
