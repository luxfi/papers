...
Per the LP-218 P3Q framework~\cite{lp218} and the roadmap
...
verbatim. A future wire version (a new value of the version byte,
e.g.\ \texttt{0x02}, registered under the same ABI and
\texttt{VerifierRegistry}) MAY carry a renamed magic such as
...
\item \textbf{It is not a kind-byte-dispatched family.} Unlike
  ...
  register an additional wire version (e.g.\ \texttt{0x02},
  post-Plonky3-upstream-rotation or a recursive variant) under the
  same ABI without re-allocating the slot, but the
