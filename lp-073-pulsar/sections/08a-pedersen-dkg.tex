...
trusted-dealer-free genesis path for the \emph{shared kernel} (the
$(c,z,\Delta)$ scheme shared with Corona). Pulsar's \emph{production}
FIPS-204 keygen is instead the dealerless Mithril RSS of
\S\ref{sec:protocol:bcc:keydkg}; both are dealerless, so no Bootstrap
path depends on a trusted dealer. The construction
replaces upstream Pulsar's Feldman commit
...
The upstream Pulsar DKG at \texttt{corona/dkg/dkg.go} uses Feldman
commits
\begin{equation}
...
Ringtail \cite{ringtail}: at least 128-bit classical and at least
