...
\texttt{0x012205} with leading \texttt{kind=0x03}. Same physical
...
LP-181~\cite{lp181}. The wrapper produces a signature
byte-identical to a single-party FIPS-205 SLH-DSA signature on
the same message and group public key; the verifier core is
therefore the FIPS-205 SLH-DSA verifier with no threshold-
specific logic.
...
routes \texttt{kind=0x03} to \texttt{abiFalse} via the
...
LP-220-conformant deployments \textbf{MUST} configure Magnetar
...
Module-LWE and Ring-LWE. Magnetar is the \textbf{last-line PQ}
