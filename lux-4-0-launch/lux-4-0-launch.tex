...
activation completes a six-year arc that began with mainnet launch in
...
resale to Zoo, Hanzo, and Pars, and the on-mainnet receipts
...
M & MPC & M-Chain threshold-signing service & FROST + Pulsar \\
...
$\le 250$~ms).
\item \textbf{Lane 3 --- Pulsar threshold} (post-quantum threshold
fallback, $\le 800$~ms).
...
synchrony, certification is guaranteed by the slowest of the
certifying lanes, and SLH-DSA continuity is preserved unconditionally.
\end{theorem}
...
documents the financial-rail core; the remaining six chains are
summarised in the 9-chain topology table above.
...
\item Cross-chain hooks via Universal Liquidity (LP-310, registered
at $h_0 + 48$).
\end{itemize}
...
template at $h_0$. A regulated exchange partner is a separate white-label deployment in
...
\midrule
Quasar 4.0 epoch boundary & $h_0$ & cert-lane vector $(\text{BLS},\text{MLDSA},\text{Pulsar},\text{SLHDSA})$ \\
...
Universal Liquidity router register & $h_0 + 48$ & router id \texttt{lp.lux.router} \\
First Pars$\to$Lux bridge tx & $h_0 + 218$ & via A-Chain attestation \\
First Hanzo$\to$Lux bridge tx & $h_0 + 305$ & via A-Chain attestation \\
First Zoo$\to$Lux bridge tx & $h_0 + 412$ & via A-Chain attestation \\
...
Lane-3 (Pulsar) participation & $84.2\%$ \\
...
all activate the same template on the same date, and the Universal
Liquidity (LP-310) common API on April 20 unifies their DEX liquidity.
...
\bibitem{lp310} Lux Network. \emph{LP-310: Universal Liquidity --- Common Settlement API}. 2026.
