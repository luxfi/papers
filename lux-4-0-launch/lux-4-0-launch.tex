...
M & MPC & T-Chain threshold-signing service & FROST + Pulsar \\
...
\item Cross-chain hooks via Liquidity Protocol (LP-310, registered
at $h_0 + 48$).
\end{itemize}
...
\texttt{Hybrid\_BLS\_Ringtail} (0x0610), both GPU-native.
...
Liquidity Protocol register & $h_0 + 48$ & router id \texttt{lp.lux.router} \\
First Pars$\to$Lux bridge tx & $h_0 + 218$ & via A-Chain attestation \\
First Hanzo$\to$Lux bridge tx & $h_0 + 305$ & via Liquidity Protocol \\
First Zoo$\to$Lux bridge tx & $h_0 + 412$ & via Liquidity Protocol \\
