...
\begin{abstract}
We present \textbf{Lux Teleport}, a non-custodial omnichain bridge and liquidity protocol that natively connects 20 execution environments and 270+ blockchains through a unified MPC threshold signature network. Unlike existing bridges that rely on trusted intermediaries, multisigs, or chain-specific messaging layers, Teleport achieves trustless cross-chain asset transfer through FROST~\cite{frost} and CGGMP21~\cite{cggmp21} threshold cryptography with on-chain signature verification, per-chain nonce tracking, and automatic undercollateralization detection.
...
\begin{enumerate}[nosep]
  ...
  \item Native bridge programs across 20~execution environments (\S\ref{sec:chains}).
...
A set $\mathcal{S} = \{s_1, s_2, \ldots, s_n\}$ of $n$ nodes, each holding a share of a distributed secret key generated through distributed key generation (DKG) with no trusted dealer. A threshold $t \leq n$ defines the minimum number of honest nodes required to produce a valid signature. Default: $n = 3, t = 2$.
\end{definition}
...
\begin{enumerate}[nosep]
  ...
  \item $G_C$ cannot change MPC signers (MPC-controlled, 7-day timelock)
\end{enumerate}
\end{property}
...
\begin{enumerate}[nosep]
  \item Current 2-of-3 MPC set signs rotation proposal with new signer addresses.
  \item 7-day timelock begins. Pending signers visible on-chain.
  \item During timelock: users can exit (burn tokens $\to$ receive underlying).
  \item After 7 days: anyone calls \texttt{executeSignerRotation()}.
  \item Current signers can cancel during timelock via \texttt{cancelSignerRotation()}.
\end{enumerate}
...
Native bridge contracts are deployed across 20 execution environments:
...
\bottomrule
\end{tabular}
\caption{20 execution environments, 270 registered chain IDs}
\label{tab:runtimes}
\end{table}
...
Users can burn bridge tokens and receive underlying assets within $\max(7\text{ days}, \text{signer\_timelock})$ under all conditions, including governor compromise and single MPC node failure.
\end{property}
...
\textbf{tBTC}~\cite{tbtc} pioneered threshold ECDSA for Bitcoin bridging. Teleport generalizes this to 20 execution environments with yield-bearing tokens and sovereign governance.
...
\bibitem{luxtp2022} Z.~Kelling and V.~Seesahai, ``Lux Teleport Protocol,'' Lux Industries, 2022. \emph{(Superseded by this paper.)}

\bibitem{luxbridge2023} Z.~Kelling and V.~Seesahai, ``Lux Bridge: Threshold-Secured Cross-Chain Communication with MPC Custody,'' Lux Industries, 2023. \emph{(Superseded by this paper.)}

\bibitem{luxthreshold2021} Z.~Kelling, ``Lux Universal Threshold Signatures,'' Lux Industries, 2021. \emph{(Companion paper: threshold crypto details.)}
