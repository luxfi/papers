...
\author{
Lux Partners\thanks{Corresponding author: research@lux.network}\\
...
Lux Consensus extends the Snow family of protocols---Slush, Snowflake, Snowball, and
Avalanche---with a directed acyclic graph (DAG) ordering layer, adaptive confidence
...
The Snow family of protocols, first described in the original Avalanche
...
or all-to-all voting rounds, Snow protocols have each node repeatedly poll a small random
...
fault tolerance for a Snow-family protocol, establishing $f < n/3$ as the exact fault
...
Snowball confidence mechanism, and VRF-based tie-breaking (\S\ref{sec:protocol}).
...
related work across the Snow family, classical BFT, and Nakamoto consensus.
...
quite dominant. The Snow family exploits this: repeated sampling amplifies any small
...
$T_c$, the system spontaneously magnetizes: one spin orientation dominates. The Snow
...
\subsection{The Snow Family}

The Snow family represents a progressive refinement of the basic random sampling idea:
...
Snowflake adds a \emph{conviction counter}~\cite{rocket2019snowflake}: an integer $cnt$
...
Snowball~\cite{rocket2019snowflake} adds a \emph{confidence counter}: a counter $d[c]$ for
each color $c$ tracking the total number of rounds with supermajority for $c$. The node's
preference is the color with the highest confidence counter. This makes Snowball harder to
manipulate: an adversary must overcome the entire history of supermajority rounds, not just
the current streak.

\subsubsection{Avalanche}
Avalanche~\cite{rocket2019snowflake} builds a DAG of transactions on top of Snowball. Each
...
Avalanche runs Snowball independently on each \emph{conflict set}---the set of transactions
...
\subsection{Snowflake Layer: Conviction Counter}

Snowflake extends Slush with a consecutive-round conviction counter:

\begin{algorithm}
\caption{Snowflake: Conviction-Based Finalization}
\label{alg:snowflake}
\begin{algorithmic}[1]
...
\subsection{Snowball Layer: Historical Confidence}

Snowball adds per-color confidence counters that persist across preference switches:

\begin{algorithm}
\caption{Snowball: Persistent Confidence Counters}
\label{alg:snowball}
\begin{algorithmic}[1]
...
Lux extends Snowball to operate over a DAG of transactions rather than binary choices.
Each transaction $t$ carries a set of parent transactions, creating a partial order. Lux
runs independent Snowball instances per conflict set and derives a consistent total
...
\item The Snowball instance for $t$'s conflict set has decided in favor of $t$,
...
With the Snowball confidence mechanism reinforcing the honest majority across rounds, the
...
The update rule for $p$ under the Snowball dynamics is:
\begin{equation}
...
timeout handling.
\item \textbf{Preference Engine}: Maintains per-conflict-set Snowball state (confidence
...
type LuxEngine struct {
    ...
    sampler    Sampler
    dag        TransactionDAG
    snowball   map[ConflictID]*SnowballState
}
...
\item \textbf{Virtuous frontier}: Non-conflicting transactions bypass the Snowball mechanism
...
The Snow-family foundation provides a qualitatively different approach to Byzantine
...
Future work includes: (1) formal verification in TLA+ and Coq, (2) adaptive parameter
tuning based on real-time network conditions, (3) integration with post-quantum signature
schemes for long-term security, (4) analysis under adaptive Byzantine adversaries, and
(5) extension to multi-value consensus beyond binary conflict sets.
