...
\item \textbf{BLS}: $\sigma_i^{\mathrm{BLS}} = \mathit{sk}_i^{\mathrm{BLS}}
      ...
      signature.
\item \textbf{Pulsar}: contribute share $\sigma_i^{\mathrm{RT}}$ to
...
(b) a valid Pulsar $\sigma^{\mathrm{RT}}$ from Q-Chain, and (c) a valid
...
\item Validator sets may change (additions, removals, stake changes);
      $P_\text{val}$ updates.
\item Q-Chain runs a new Pulsar DKG; $Q_\text{cer}$ updates and
