...
Groth16 & Prove ($N{=}21$ ML-DSA)    & 200\,ms            & 5--15\,ms     &
        $\approx 2^{22.5}$ R1CS \\
\midrule
Corona & Round~1 share              & 1.8\,ms            & ---           &
        precomputable \\
Corona & Round~2 sign+combine       & 9\,ms              & ---           &
        per round \\
Corona & Batch verify               & $O(1)$ amortized   & ---           &
        cached after DKG \\
\bottomrule
\end{tabular}
\caption{Per-lane verification primitives. The Groth16 verify step is
the dominant cost on the GPU verification path; Corona batch verify
...
release. Latency is the parallel max of BLS aggregate, Corona batch
...
$n{=}21$. Block time is therefore signing-bound (Corona two-round
...
\midrule
Corona   & DKG (5-of-7)    & 12.4\,ms    & ---       & ${\sim}56$\,KB \\
Corona   & Round~1 (precomp) & 1.8\,ms    & ---       & --- \\
Corona   & Round~2 (sign)  & 2.1\,ms     & ${\sim}56$\,KB & --- \\
...
QuasarCert footprint of 56.3\,KB is dominated by the Corona signature;
in epoch mode (one Corona per $k$ blocks), the per-block amortized
