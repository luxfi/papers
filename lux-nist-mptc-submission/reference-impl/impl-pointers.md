...
| `cmd/corona_oracle_v2/` | KAT oracle: `emit --out <dir>` produces 16 deterministic KAT entries. |
...
- Pulsar SUF-CMA: mechanized in Lean (`lean/Crypto/Corona.lean`).
- Reshare preserves master secret: mechanized in Lean (`lean/Crypto/Threshold/Reshare.lean`).
- Triple consensus (BLS + Corona + ML-DSA): mechanized in Lean (`lean/Consensus/Quasar.lean`).
...
The implementation is byte-equal across the canonical Go and the in-flight C++ port (`luxcpp/crypto/corona/cpp/`); KAT vectors gate every release.
