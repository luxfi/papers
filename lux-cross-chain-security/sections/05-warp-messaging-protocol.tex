...
This section describes the Warp 1.x bare BLS-aggregate format. The
current shipping protocol is \textbf{Lux Warp 2.0} (LP-021v2 /
LP-6022), a hybrid envelope that carries the Warp 1.x message intact
(the \emph{Beam} lane below) alongside an ML-DSA cert set and a
Pulsar Pulse (Lux's Ringtail variant, see LP-073) for post-quantum
verification --- all three lanes bound to a common source-chain
transcript. Reference implementation:
\texttt{github.com/luxfi/warp} (\texttt{envelope.go},
\texttt{pulsar/}). The remainder of this section is the Beam
specification.
