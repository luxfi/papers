...
Pulsar threshold signatures use lattice-based shares \cite{ringtail}. The share combination involves polynomial addition over the ring $\mathbb{Z}_q[X]/(X^N+1)$. Due to the larger share size ($\sim$1\,KB vs. 32\,B for FROST-BLS), HSM storage is a concern for large threshold sets. TEE-based execution (with attestation) is the preferred boundary for Pulsar shares.
