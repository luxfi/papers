...
export function PaperLibrary({ papers }: { papers: readonly PaperConfig[] }) {
  const [query, setQuery] = useState('')
  const [category, setCategory] = useState('All')
  const categories = useMemo(() => ['All', ...Array.from(new Set(papers.map((paper) => paper.tags[0]))).sort()], [papers])
  const shown = useMemo(() => {
    const needle = query.trim().toLowerCase()
    return papers.filter((paper) => {
      const categoryMatches = category === 'All' || paper.tags.includes(category)
      const queryMatches = !needle || [paper.title, paper.subtitle, paper.abstract, ...paper.tags, ...paper.authors].join(' ').toLowerCase().includes(needle)
  ...
  return (
    <section>
      <div className="mb-8 grid gap-3 rounded-lg border border-border/60 bg-card p-3 md:grid-cols-[1fr_auto]">
        ...
        <select value={category} onChange={(event) => setCategory(event.target.value)} aria-label="Filter by category" className="rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-foreground/50">
          {categories.map((value) => <option key={value}>{value}</option>)}
        </select>
      </div>
      <div className="mb-4 flex items-center justify-between text-sm text-muted-foreground">
        <p>{shown.length} {shown.length === 1 ? 'paper' : 'papers'}</p>
        <p>{papers.filter((paper) => paper.featured).length} canonical selections</p>
      </div>
