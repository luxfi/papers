...
sustains 9.13\,B byte-exact \texttt{uint256} orders/sec on an NVIDIA
GB10, 12.76\,B on an AMD Radeon 8060S, and 5.60/2.80\,B on Apple
M4 Max/M1 Max; the throughput levers are a coalesced structure-of-arrays
arena layout ($\sim$6$\times$ over an un-coalesced baseline) and a
Knuth Algorithm-D division that replaces a 512-iteration bit-serial
\texttt{uint256} VWAP, with output byte-identical to the CPU
oracle (LP-203).
...
\begin{itemize}[nosep]
  ...
  \item FHE for front-running prevention $\rightarrow$ FHE
        ...
        on the DEX roadmap (Section~\ref{sec:roadmap}).
\end{itemize}
