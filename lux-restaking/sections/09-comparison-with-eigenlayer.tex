...
\paragraph{Cross-chain: Lux Teleport bridge (LP-6332).}
BTC locked on Bitcoin L1 is attested by an MPC watcher network (threshold $t$-of-$n$,
$n \geq 15$, $t \geq 10$).  Upon confirmation, LBTC is minted on Lux via the Lux Teleport
protocol~\cite{lux2025teleporter}, which rides on Lux Warp 2.0 (Beam BLS aggregate +
ML-DSA cert set + Pulsar Pulse, see \texttt{github.com/luxfi/warp}).  Yield accrued by
the Babylon finality provider flows back to Lux through a \texttt{mintYield()} callback
on the Lux Teleport bridge contract, credited to the \texttt{BabylonBTCStrategy} vault.
...
\midrule
ETH   & Lux Teleport (MPC) & EigenLayer restaking & \texttt{EigenETHStrategy} \\
BTC   & Lux Teleport (MPC) & Babylon finality provision & \texttt{BabylonBTCStrategy} \\
...
Cross-chain & Lux Teleport & EVM bridges & BTC $\to$ PoS & EVM only \\
