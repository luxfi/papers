...
\item Corona's verify dominates a single core because the NTT
  ...
  Ringtail-parameter coefficients is a tight cache-resident loop with
...
gas in a future amendment; LP-219 does not codify it.
...
Reserved-but-not-wired kind & caller passes \texttt{kind=0x02}
  or \texttt{kind=0x03} before verifier lands &
  precompile returns \texttt{abiFalse}; caller observes \texttt{false} \\
