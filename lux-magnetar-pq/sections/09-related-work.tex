...
\paragraph{Corona (Lux LP-104)
\cite{lp104corona,ringtail}.}
The other companion. Corona is a Module-LWE threshold variant at
the Ringtail-derived parameter set (module matrix
$A \in R_q^{8\times7}$ over $R_q = \mathbb{Z}_q[X]/(X^{256}+1)$).
Same public-BFT-safe threshold mechanics as Pulsar. Corona's
trade-off vs. Pulsar: a different Module-LWE parameter regime and
codebase (Ringtail-derived rather than ML-DSA-65), larger
signatures, slightly slower signing.
...
secp256k1, P-256, and ristretto255. FROST's linear share-aggregation
identity $z = \sum_i z_i$ is the
