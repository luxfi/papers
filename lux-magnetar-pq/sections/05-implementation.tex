...
\texttt{v1.0.0}. The artefact ships both the per-validator standalone
primary primitive
(\texttt{ref/go/pkg/magnetar/standalone.go} and \texttt{wire.go})
and the THBS-SE permissionless threshold companion
(\texttt{ref/go/pkg/magnetar/thbsse.go} and
\texttt{thbsse\_field.go}). This section maps the
per-validator standalone construction of \S\ref{sec:construction}
and the THBS-SE public-combiner Combine path of \S\ref{sec:thbsse} to
the source layout.

\subsection{Public-combiner Combine path}
\label{sec:impl:combine-path}

The THBS-SE Combine path in v1.0 is implemented at
\texttt{ref/go/pkg/magnetar/thbsse.go::Combine}, backed by the
byte-wise $\mathrm{GF}(257)$ Shamir arithmetic in
\texttt{thbsse\_field.go} (\texttt{thbsseReconstructGF},
\texttt{thbsseModInvSmall}, \texttt{lagrangeBasisAtZero}). The
public combiner Lagrange-reconstructs the SLH-DSA scheme seed in
its own transient memory, dispatches to
\texttt{cloudflare/circl/sign/slhdsa.SignDeterministic}, and
zeroizes the seed and every working buffer before return
(\S\ref{sec:thbsse:combine}).

This is the v1.0 honest open item: the reconstructed seed exists in
the combiner's address space for the duration of one
\texttt{SignDeterministic} call (sub-second wall-clock at
SHAKE-192s). The combiner role is public --- any peer can run it ---
and no host is in the TCB by policy (\S\ref{sec:security:thbsse-tcb}).

The strict ``no transient seed at any moment'' refinement is the
planned v1.1 lift (\texttt{MAGNETAR-STRICT-ATOM-V11}): a
Magnetar-internal FIPS 205 \S5--\S8 walk that assembles the
signature directly from per-atom reconstructions of the
message-selected FORS leaves and WOTS+ chain bases, bypassing
\texttt{slh\_sign\_internal} so the master seed is never
materialised under a named \texttt{seed} / \texttt{SK.seed} /
\texttt{SK.prf} variable in the public combiner's scope. Closing
the residual gap entirely (no master-expansion bytes transient at
any moment) requires either full MPC over the SHAKE-256 hash tree
(open research) or a TEE-attested host in the TCB (sibling
primitive at \texttt{luxfi/threshold/protocols/slhdsa-tee}). The
wire format, share format, slot-guard state, and protocol round
structure are all forward-compatible with the lift; only Combine's
internals change.
...
\begin{table}[H]
\centering
\caption{Magnetar v1.0.0 file layout. The per-validator standalone
primary path lives in \texttt{standalone.go}; the THBS-SE
permissionless threshold companion lives in \texttt{thbsse.go} and
\texttt{thbsse\_field.go}, and is documented in
\S\ref{sec:thbsse} and \S\ref{sec:impl:combine-path}.}
...
\texttt{thbsse\_field.go}  & Internal GF(257) byte-wise Shamir share arithmetic; \texttt{thbsseReconstructGF}, \texttt{lagrangeBasisAtZero}, \texttt{MaxCommittee257}, \texttt{EvalPointFromID} \\
\midrule
...
through the v1.0 per-validator standalone primary path:
