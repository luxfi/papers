...
\textbf{Classical blockchain} (ECDSA):
\begin{equation}
\text{Sig}_{sk}(m) = \text{ECDSA}(sk, m) \quad \text{(32 bytes)}
\end{equation}

\textbf{Quantum consensus} (ML-DSA):
\begin{equation}
\text{Sig}_{sk}(m) = \text{ML-DSA}(sk, m) \quad \text{(3,293 bytes)}
\end{equation}

\textbf{Challenge}: 100$\times$ larger signatures impact network bandwidth.

\textbf{Solution}: Aggregation via BLS-like lattice schemes.
