...
Corona \cite{ntt-corona} provides lattice-based threshold signatures.
...
\State \quad $QuantumSig$: Corona signature
...
\item GPU acceleration for Corona lattice operations
...
Corona & 15-of-21 (PQ) & LWE ($n=630$, $q=2^{32}$) & $< 2^{-128}$ \\
Dual-sig & CGG21 + Corona & ECDLP $\times$ LWE & $< 2^{-255}$ \\
\bottomrule
\end{tabular}
