...
marginal price $m(t)$ starts at the peg and is moved by trades. In the model the
operator is the dominant LP and runs the arbitrage loop; external flow is exogenous.
...
\paragraph{Cycle economics.} When the LP and the arber are the same operator, arb fees and arb
adverse selection are internal washes. Real income is the fee on \emph{external} volume
$V_{\text{ext}}$; real cost is the base-fee burn of requoting and of the operator's arb swaps:
