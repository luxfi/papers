\section{Comparison with Other DAOs}


\begin{table}[h]
\centering
\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Metric} & \textbf{Lux (target)} & \textbf{Maker} & \textbf{Uniswap} & \textbf{Compound} \\ \midrule
Avg Participation & 18.3\% & 4.2\% & 3.8\% & 6.1\% \\
Proposals/Year & 94 & 52 & 28 & 31 \\
Avg Vote Duration & 7.2 days & 14 days & 7 days & 3 days \\
Treasury Value & \$128M & \$8.2B & \$2.1B & \$850M \\
Quantum-Secure & Yes & No & No & No \\
Cross-Chain & Yes & No & No & No \\ \bottomrule
\end{tabular}
\caption{DAO comparison metrics. Lux figures are design targets (see \S1); Maker, Uniswap, and Compound figures are approximate public data.}
\end{table}

