...
The on-chain substrate for Lux Governance is the Azorius modular framework: a
Safe treasury with a Zodiac-pattern Azorius execution module and pluggable
voting-strategy contracts, augmented with ERC-4337 gasless voting, Hats Protocol
roles, and ERC-6551 token-bound sub-accounts. That substrate is specified in the
companion \emph{Lux DAO: Modular Governance Framework} paper and is the canonical
governance implementation. The interfaces in this section describe the
Lux-specific coordination and voting-strategy logic layered on top of that
Azorius substrate --- proposal typing, validator-weighted votes, and
cross-ecosystem aggregation --- rather than a replacement for it. All execution
resolves through the Safe; all role gating resolves through Hats.
...
contract QuantumSecureVoting {
    ...
    function verifyVote(
        ...
        bool pulsarValid = verifyPulsar(
            voter, message, sig.pulsarSignature
        );

        return blsValid && pulsarValid;
    }
