...
Corona (Module-LWE threshold), $0x03$ Magnetar (FIPS-205 SLH-DSA
...
slot with kind-byte dispatch (LP-219 \S 2,
``Architectural Decision -- single slot, kind-byte dispatch'').
One slot, one ABI, three kinds.
...
\item \textbf{Constant-time.} No data-dependent branches; no side
  channels.
\item \textbf{Slot.} \texttt{0x012205} per LP-219.
...
\begin{tabular}{@{}lrll@{}}
\toprule
\textbf{Verifier} & \textbf{Gas} & \textbf{Verify time} & \textbf{Notes} \\
\midrule
...
operations on every CPU. The $10\times$ factor is
honest, not pessimistic. The production P3Q verify runs on the
pure-Go FIPS-204 verify path (CPU, no GPU kernel), and the gas
accounting reflects that CPU-class verify cost as the calibration
baseline; a projected GPU batched-verify path (LP-203) would narrow
the absolute-time gap at scale but does not change the gas
calibration.
...
Pulsar verifier core & \texttt{lux/pulsar/verify/} & Constant-time FIPS 204 verifier -- P3Q wraps this (production verify is pure-Go/CPU) \\
GPU kernel (projected) & \texttt{lux/accel/cuda/p3q.cu} & Projected LP-203 batched-verify path; not on the production verify path \\
\bottomrule
\end{tabular}
...
\subsection{One wired kind today; two reserved}

P3Q is the unified PQ-verifier family, but only the Pulsar kind
(\texttt{0x01}, FIPS-204 ML-DSA) is wired into a live verifier
today. The Corona (\texttt{0x02}, Module-LWE) and Magnetar
(\texttt{0x03}, FIPS-205 SLH-DSA) kinds are allocated in the ABI
and reserved on the same slot; until their verifiers are wired,
those kind values return \texttt{abiFalse} per the kind-dispatch
property above. The single-slot, kind-byte design means activating
them is a verifier wiring change behind a stable ABI, not a new
precompile slot.
...
produce Pulsar signatures, so it is the kind worth wiring first. A
proliferation of separate PQ-verifier slots would expand the
surface area for no immediate benefit; the unified family avoids
that. When Corona-threshold-signed rollups become operationally
relevant, LP-220 wires the Corona verifier into P3Q kind
\texttt{0x02} on this same slot.
