...
canonical CBOR/COSE manifest~\cite{rfc8949,rfc8152}. We pair the envelope
with the Lux \textbf{Z-Wing} secure channel
(luxfi/zwing~\cite{luxzwing}) for transport so that endpoint authentication,
key exchange, and replay protection are end-to-end post-quantum, with
optional Reticulum Network Stack (RNS, LP-9701~\cite{lp9701}) routing for
disconnected, mesh, or LoRa deployment. The whole stack is FIPS/CNSA-2.0
...
\item \textbf{Forward secrecy at the channel layer.} Z-Wing uses
  ephemeral X-Wing per session and AEAD nonce sequence numbers so
  past sessions cannot be decrypted if a long-term identity key is
  later compromised.
\end{enumerate}
...
\subsection{Perfect forward secrecy for AEAD}
\label{sec:pfs}

Z-Wing and the AEAD chunk layer both provide forward secrecy:

\begin{itemize}[leftmargin=*,itemsep=0pt]
\item \textbf{Channel (Z-Wing).} For every session, both peers
  generate a fresh X-Wing keypair; the ephemeral private keys are
  zeroed after the shared secret is derived. The session key derives
  from $\textsf{HKDF}(\textsf{X-Wing shared}\,\|\,\textsf{transcript})$.
  A future compromise of either long-term identity key
  (Ed25519 \emph{or} ML-DSA-65) cannot decrypt recorded sessions: the
  ephemeral component is gone. Both legs must break for retrospective
  decryption.
\item \textbf{Chunk AEAD.} Each 64-KiB chunk is encrypted with a
  unique nonce $(N_0\,\|\,\text{seq})$ where $N_0$ is a 12-byte random
  nonce-seed bound into the manifest and $\text{seq}$ is a 4-byte
  big-endian chunk index. The DEK itself is rotated every $2^{32}$
  chunks (256~TiB) via HKDF re-keying chained from the manifest's
  ratchet seed; a compromise of the DEK does not decrypt chunks
  emitted before the most recent re-key. For interactive transfer
  this is irrelevant (files fit under the rotation limit); for
  long-lived replication streams (Section~\ref{sec:replication}) it
  prevents one DEK compromise from leaking the entire history.
\end{itemize}
