...
QuasarCert: luxfi/corona (Module-LWE, Ringtail-derived) and luxfi/pulsar (Module-LWE)
...
application protocols (file transfer, ZAP RPC, HTTP/2, anything
...
\bottomrule
\end{tabular}
\caption{Wire-format growth (per LP-9701 measurements).}
\end{table}
...
\texttt{luxfi/corona} & Module-LWE threshold signature kernel (Ringtail-derived). Recovery quorum and federation root signing. \\
...
\texttt{hanzoai/iam} & Identity provider: certificate issuance, recipient public-key lookup, revocation registry. \\
...
\textbf{Source code.} \texttt{github.com/luxfi/pqsafe} (envelope tool, commit \texttt{TBD}); \texttt{github.com/luxfi/age} (PQ-extended age fork); \texttt{github.com/luxfi/zwing} (identity / signing); \texttt{github.com/luxfi/pqrns} (Reticulum PQ profile).\\
\textbf{Build.} \texttt{go build ./...} for pqsafe / zwing / pqrns; \texttt{cargo build --release} for the age fork.\\
\textbf{Hardware tested.} TBD (commodity x86\_64 Linux with AES-NI; Apple Silicon).\\
\textbf{Standards referenced.} FIPS 140-3, FIPS 180-4 (SHA-2), FIPS 197 (AES), FIPS 202 (SHA-3/SHAKE), FIPS 203 (ML-KEM-768), FIPS 204 (ML-DSA-65), FIPS 205 (SLH-DSA-SHA2-192f), NIST SP 800-38D (GCM), NIST SP 800-56C (KDF), NIST SP 800-208 (stateful hash-based), RFC 8032 (Ed25519), RFC 9180 (HPKE), CNSA 2.0.\\
