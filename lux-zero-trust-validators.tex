...
A Lux validator consists of multiple cooperating components: the \texttt{luxd} node process, EVM plugin processes, signer services (for BLS and Corona keys), metrics exporters, and operational tooling. In a traditional deployment, these components trust each other implicitly---they share a filesystem, a network namespace, or a pod boundary. This implicit trust creates lateral movement opportunities: a compromised metrics exporter can access the signing key, a bug in an RPC handler can reach the consensus engine.
...
This paper describes how these principles are applied to Lux validator operations, with particular focus on the signer service---the component that holds the validator's BLS and Corona private keys and must be protected above all else.
...
\begin{itemize}
\item No network access except a Unix domain socket (or mTLS-authenticated gRPC).
