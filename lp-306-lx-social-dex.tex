...
(\texttt{luxfi/broker}), and the Liquidity ATS, and identifies
the features that shipped, evolved, or were superseded by superior
alternatives.
\end{abstract}
...
\textbf{2026}: The Liquidity ATS (\texttt{luxfi/ats})
implements the regulated version: 12,784 SecurityTokens on
the Liquidity EVM, portfolio management via the exchange
frontend (\texttt{luxfi/exchange}), and social trading
features (leaderboards, copy-trade, rebalancing) designed
and approved for the next milestone. The permission-mapping
...
\begin{itemize}[nosep]
  ...
  \item Zero-knowledge file system $\rightarrow$
        ...
        (E2E encrypted blind blob store, shipped April 2026).
  \item FHE for front-running prevention $\rightarrow$ FHE
        CRDT merge (shipped April 2026), encrypted orderbook
...
The current Lux Network uses a fixed supply of 10B LUX (for the
Liquidity L1: 10B LQDTY) with no inflation. Validator rewards
come from transaction fees, not token emission. The
equation-of-exchange model was not adopted --- the market moved
toward fee-burn models (EIP-1559 style) rather than
inflation-funded staking.

The RWA-backed stable currencies (GLUX, ULUX pegged to gold/uranium)
described in the 2022 paper were superseded by USDL (1:1 USD-backed
via Braintree/ACH deposits) on the Liquidity EVM.
...
Social profiles + stats   & Designed & Liquidity exchange v2 \\
Copy trading              & Designed & ATS social trading milestone \\
People-Based Portfolios   & Designed & ATS rebalancing milestone \\
...
RWA tokens                & Shipped  & 12,784 SecurityTokens on Liquidity EVM \\
...
Lux Broker (16-venue smart order router), and the Liquidity ATS
(regulated ATS with 12,784 SecurityTokens). The social trading
