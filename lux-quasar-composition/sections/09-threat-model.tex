...
\item Per-leg primitive unforgeability (delegated to per-primitive
      papers; see \cite{pulsar,corona,magnetar,
      quasarcertsoundness}).
\end{itemize}
...
\item Validator-set rotation: governed by the LSS framework
      ...
      (\cite{pulsar} for Pulsar, etc.). The cert sees only the
...
(Module-LWE) is independent and still valid (Corollary~\ref{cor:polaris}).
...
Module-LWE (some breakthrough sieving / NTT-aware attack). On the
