...
\item The Pulsar leg is produced by the Pulsar two-round threshold
      protocol \cite{pulsar}: Round-1 commitments
...
\item The Corona leg (on Aurora / Polaris) follows the same
      two-round structure on the Module-LWE ring $R_q$
      \cite{corona,ringtail}.
\item The Magnetar leg (on Polaris) follows the v0.1 reveal-and-aggregate
      protocol \cite{magnetar}: validators emit Shamir shares of
...
This is critical because Lux supports mixed-mode L1 topologies:
...
\cite{pulsar}; Corona will do the same once its Tier A
documentation completes (current status Tier A docs landed
\cite{corona}); Magnetar's reshare semantics under the v0.1
