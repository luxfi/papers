...
signature, a Corona threshold M-LWE signature, a Magnetar threshold
...
\item LP-073-Corona / Corona~\cite{ringtail,corona}
      defines the threshold Module-LWE primitive (Lux production fork of
...
demands additionally a break of Corona (M-LWE~\cite{regev2009,
...
operation twice. The Pulsar and Corona legs diversify \emph{within}
the lattice family --- distinct Module-LWE parameter sets and
codebases, so a parameter- or implementation-localized break need
not hit both --- while the Magnetar leg diversifies \emph{across}
families: a break against the lattice family as a whole does not
transfer to hash-based constructions. Family-disjoint hardness comes
from Magnetar, not from pairing two Module-LWE legs. The combinatorial
hardness reduction in Section~\ref{sec:composition} formalises this
claim under standard reductions.
