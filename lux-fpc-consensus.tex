...
\author{Zach Kelling\thanks{Corresponding author: zach@lux.network} \quad Woo Bin\\
...
\begin{itemize}
\item We formalize the Fast Probabilistic Consensus (FPC) protocol with rigorous safety and liveness proofs, verified in Lean~4 with machine-checked proofs~\cite{luxformal2026}.
...
Wave (fixed-$\theta$)~\cite{rocket2018snowflake} & $O(k \log n)$ & Probabilistic & 99.9\% & No \\
...
Fixed-threshold sampling protocols~\cite{rocket2018snowflake} use a constant decision threshold which can cause metastability. FPC's adaptive thresholds prevent oscillation while maintaining similar message complexity.
...
FPC's safety and liveness properties have been formally verified in Lean~4, with machine-checked proofs available at \texttt{github.com/luxfi/formal}~\cite{luxformal2026}.
...
\bibitem{luxformal2026}
Kelling, Z. \& Bin, W. (2026). Formal Verification of Lux Consensus in Lean~4. \texttt{github.com/luxfi/formal}. Machine-checked proofs of safety, liveness, and BFT tolerance for Wave/FPC consensus.
