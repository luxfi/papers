...
MPC signer rotation is the highest-severity operation. The protocol enforces:

\begin{enumerate}[nosep]
  \item Current $t$-of-$n$ MPC set signs the rotation proposal.
  \item A 7-day timelock begins. Pending signers are published on-chain.
  \item During the timelock, users may exit by burning bridge tokens for underlying assets.
  \item After 7 days, any address may execute the rotation.
  \item Current signers may cancel during the timelock period.
\end{enumerate}

\begin{property}[Exit Window]
\label{prop:exit}
For any signer rotation event at time $t_0$, every bridge token holder has the interval $[t_0, t_0 + 7 \text{ days})$ to exit the system by burning bridge tokens and receiving the underlying asset at the current share price.
\end{property}
