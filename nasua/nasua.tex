...
Takahashi, and Tibouchi~\cite{ringtail} verbatim, which
...
Module-LWE fork of Annulus. Lifts the Ring-LWE ring $\Rq$ to the
Module-LWE vector ring $\Rq^k$, switches the hash family to
SHA-3 / SP~800-185, adds identifiable abort, adds proactive resharing
with beacon-randomised quorum selection, and ships through the
Pulsar-SHA3 hash suite.
\item \textbf{Pulsar}~\cite{pulsar} --- the Module-LWE /
FIPS~204-compatible expansion that tunes Corona's $\Rq^k$
parameters to ML-DSA so the threshold output is
...
\textbf{Corona} & Module-LWE (Lux production) & NIST MPTC S1 + S4 & production-enabled \\
...
Nasua follows the original Boschini et al construction~\cite{ringtail}:
...
profiles for an M-LWE submission. Corona substitutes
...
\begin{center}
\begin{tabular}{r c l}
\textbf{Nasua} & $\xrightarrow{\text{harden + lift R-LWE} \to \text{M-LWE}}$ & \textbf{Corona} \\
                 &                                & \quad $\downarrow$ \\
                 &                                & \quad $\text{(FIPS~204 ML-DSA tuning)}$ \\
                 &                                & \quad $\downarrow$ \\
                 &                                & \textbf{Pulsar} \\
\end{tabular}
\end{center}

Corona keeps Nasua's 2-round sign skeleton, lifts the Ring-LWE
algebra ($\Rq$, single polynomial) to Module-LWE
($\Rq^k$, polynomial vectors), replaces the hash
family, adds Pedersen-DKG, adds proactive reshare, adds
identifiable abort, and gates the protocol's wire formats through
the Lux Pulsar-SHA3 hash suite. Pulsar then tunes the Module-LWE
parameters to FIPS~204 so that the aggregated signature is
...
rename). It dispatches to Corona's M-LWE verifier for the production
...
\newblock \emph{IACR ePrint 2024/1113}; IEEE S\&P 2025, 2024.
...
\bibitem{corona}
Lux Industries Inc.
\newblock Corona: Production Module-LWE threshold signatures for the Lux
primary network.
