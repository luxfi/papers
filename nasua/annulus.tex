% Annulus: Ring-LWE Threshold Signatures
...
\title{Annulus\\
       ...
       Academic upstream of the Lux Quasar family\\
       \normalsize (code-name \texttt{Nasua}; \texttt{github.com/luxfi/nasua})}
\author{Lux Industries Inc.}
\date{Status: academic-only; superseded by Corona for production deployment.}

\begin{document}
\maketitle

\begin{abstract}
Annulus is the Lux name for the Ring-LWE 2-round threshold-signature
primitive introduced as ``Ringtail'' by Boschini, Kaviani, Lai,
Malavolta, Takahashi, and Tibouchi~\cite{boschini2024ringtail},
which itself builds on the Raccoon Fiat--Shamir-without-rejection
...
threshold-quorum ceremony per signature. The Lux fork that this
paper documents ships under the implementation code-name
\texttt{Nasua} (\texttt{github.com/luxfi/nasua}) and tracks the
academic construction verbatim; it is the upstream-tracking
variant in the Lux Quasar family.

Annulus is \emph{not enabled} in Lux production. The production
R-LWE path is \emph{Corona}~\cite{corona}, a hardened fork of
Annulus that switches the hash family to SHA-3 / SP~800-185, adds
identifiable abort, adds proactive resharing with
beacon-randomised quorum selection, and ships through the Lux
Pulsar-SHA3 hash suite. The Module-LWE / FIPS~204-compatible
expansion is \emph{Pulsar}~\cite{pulsar}, which lifts the same
two-round protocol from $\Rq$ to $\Rq^k$ so the threshold output
is byte-identical to a single-party FIPS~204 ML-DSA signature.
\end{abstract}
...
\textbf{Annulus}~(this paper) & Ring-LWE (academic upstream) & academic & \emph{not enabled} \\
\bottomrule
\end{tabular}
\end{center}
...
``ring of fire'' of an annular eclipse). Annulus is the family's
academic root and the name retains the ring-shape connection to
its Ringtail upstream while distinguishing the Lux upstream-tracker
from production deployments.

\section{Construction summary}
\label{sec:construction}

Annulus follows the original Ringtail construction~\cite{boschini2024ringtail}:
...
\item DKG: trusted-dealer keygen in the original paper. Annulus
inherits this directly; the trusted-dealer assumption is the
single hardest blocker to production deployment.
\item Reshare: not present. Annulus is a fresh-keygen-per-epoch
primitive.
\end{itemize}

\section{Why Annulus is academic-only}
\label{sec:academic-only}

Three concrete blockers prevent direct deployment of Annulus on the
Lux primary network:

\begin{enumerate}[leftmargin=*, itemsep=0pt]
\item \textbf{Trusted-dealer DKG.} The academic Ringtail
...
hash discipline.
\item \textbf{No proactive resharing.} Annulus is fresh-keygen-only.
...
\begin{center}
\begin{tabular}{r c l}
\textbf{Annulus} & $\xrightarrow{\text{harden}}$ & \textbf{Corona} \\
                 &                                & \quad $\downarrow$ \\
...
Corona keeps Annulus's 2-round sign skeleton, replaces the hash
...
The Annulus reference implementation lives in
\texttt{github.com/luxfi/nasua}. The repository tracks the academic
upstream and ships KAT vectors against the original Ringtail
construction. No precompile, no consensus integration: Annulus is
...
\texttt{0x0802} is named \texttt{Ringtail} (wire-stable; do not
rename). It dispatches to Corona's R-LWE verifier for the production
path. Annulus signatures (BLAKE3-bound transcripts, R-LWE) do not
verify under this precompile.

\paragraph{Cross-implementation conformance.} Annulus KAT vectors
are byte-stable against the upstream Ringtail reference. Corona's
KAT vectors are byte-stable against the Pulsar-SHA3 hash suite and
are not interchangeable with Annulus's. This is intentional: the
hash family substitution is what makes Corona a Class~S NIST
submission candidate; Annulus is the research-only complement.
...
M.~Tibouchi.
\newblock Ringtail: Practical two-round threshold signatures from
