...
canonical QBlock, and produces a Pulsar-M finality certificate over the
QBlock digest.
...
19 & \texttt{PrevFinalityCert}        & previous epoch's Pulsar-M cert digest. \\
...
The Pulsar-M finality lane (Section~\ref{sec:pulsar-m}) signs over this
...
certificate trailer is the Pulsar-M aggregate plus committee bitmap, sized
...
1{,}080 & varies & \texttt{finality\_cert} (Pulsar-M-65 aggregate + bitmap) \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Finality cert binding to Pulsar-M}

The Pulsar-M finality lane (Section~\ref{sec:pulsar-m}) operates as a
...
\begin{enumerate}[leftmargin=2em,nosep]
  ...
  \item Verifies the Pulsar-M aggregate signature over the digest under the
