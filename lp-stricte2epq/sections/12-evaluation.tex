...
\texttt{luxfi/pulsar}, \texttt{luxfi/p3q@v0.0.1}. The benchmark
...
Pulsar-65 (threshold) & 1{,}952 B / 60.6 KB & 3{,}309 B & finality lane (byte-eq ML-DSA-65) \\
...
\begin{itemize}[leftmargin=2em,nosep]
  \item Z-Chain rollup batching: \texttt{OpCommitTxAuthBatch} commits a
    ...
    bytes per envelope.
  \item Pulsar aggregation: $n$ Pulsar partial signatures aggregate
...
\subsection{Pulsar throughput}

\begin{table}[H]
\centering
\caption{Pulsar-65 round latency vs committee size and threshold.
...
At an illustrative single committee of $n=64, t=43$, the total round
latency is 91 ms (the canonical model instead samples smaller $n{=}8$,
$t{=}7$ committees with an $r$-of-$m$ acceptance rule; see
\S\ref{sec:pulsar}). A 100-ms round budget is therefore comfortable for
sub-second finality on a chain producing 1-second blocks.
...
which $\approx 1.3$ seconds is the Pulsar and p3q work.
...
\begin{itemize}[leftmargin=2em,nosep]
  ...
  \item Equivalent or better gas cost than classical ECDSA when batched
    through Z-Chain.
  \item $91$-ms Pulsar round at the production committee size.
\end{itemize}
