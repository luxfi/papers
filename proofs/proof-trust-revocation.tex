...
\title{Formal Verification: Revocation Model}
...
A revocation record: revoker, revoked, timestamp, signature validity.
\end{definition}

\begin{definition}[VouchRef]
A vouch reference: voucher and vouchee.
\end{definition}


\section{Theorems and Axioms}
\begin{theorem}[\texttt{unsigned\_revocation\_ignored}]
Revocations with invalid signatures are ignored.
\end{theorem}

\begin{theorem}[\texttt{revocation\_monotone}]
Adding a revocation preserves existing revocations: if vouch was revoked before, it remains revoked.
\end{theorem}
\begin{proof}
By \texttt{List.any\_cons} and \texttt{Or.inr}.
\end{proof}

\begin{theorem}[\texttt{empty\_revokes\_nothing}]
An empty revocation list revokes nothing.
\end{theorem}

\begin{theorem}[\texttt{valid\_revocation\_works}]
A correctly signed revocation for the exact vouch succeeds.
\end{theorem}

\begin{theorem}[\texttt{valid\_filter\_preserves}]
Filtering to only valid (signed) revocations preserves revocation status.
\end{theorem}


\section{Lean Source}
\lstinputlisting[language=Lean]{../../formal/lean/Trust/Revocation.lean}

\section{References}
\noindent Source code: \url{https://github.com/luxfi/proofs/blob/main/lean/Trust/Revocation.lean}\\
