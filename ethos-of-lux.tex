...
\medskip\noindent\textit{2026 status}: Quasar consensus (LP-020)
...
FROST, and Pulsar~\cite{lux-corona} as independent protocol modules.
...
(ML-DSA, ML-KEM, Pulsar)~\cite{lp305} is deployed at the consensus layer, not
...
schemes (ML-DSA-65, SLH-DSA, Pulsar) are available as EVM
...
\medskip\noindent\textit{2026 status}: The GPU-accelerated CLOB matching
engine sustains 9.13\,B byte-exact \texttt{uint256} orders/sec on an NVIDIA
GB10 and 12.76\,B on an AMD Radeon 8060S (5.60\,B on an Apple M4 Max, 2.80\,B
on an Apple M1 Max), byte-identical to the CPU reference~\cite{lp306}. Quasar
consensus scales linearly with validator count up to tested limits of 1,000
nodes.
...
(Quasar~\cite{lp105}: BLS $\wedge$ ML-DSA $\wedge$ Pulsar, Christmas 2025).
...
\bibitem{lux-corona}
\textit{luxfi/corona}: Module-LWE threshold signature scheme (Ringtail-derived).
...
Pulsar). \url{https://github.com/luxfi/consensus}.
