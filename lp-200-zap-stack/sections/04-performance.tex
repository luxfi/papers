...
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Operation} & \textbf{Reflection codec} & \textbf{ZAP Stack} & \textbf{Win} \\
\midrule
Parse one transaction & \SI{4980}{\nano\second}, 51 allocs & \SI{68}{\nano\second}, 1 alloc & $73\times$ \\
Validate a \num{1000}-tx block & \SI{4.81}{\milli\second} & \SI{0.069}{\milli\second} & $70\times$ \\
Validate a \num{5000}-tx block & \SI{24.2}{\milli\second} & \SI{0.35}{\milli\second} & $70\times$ \\
Produce a \num{1000}-tx block & \SI{1.74}{\milli\second} & \SI{0.31}{\milli\second} & $5.7\times$ \\
Allocations per block validate & \num{51000} & \num{1000} & $51\times$ \\
Memory held, live \num{1000}-tx block & \SI{1990}{\kilo\byte} & \SI{17}{\kilo\byte} & $114\times$ \\
Corona aggregate $n{=}64$ (incorrect serial baseline) & \SI{14}{\second} & \SI{356}{\milli\second}--\SI{440}{\milli\second} parallel & $-97\%$ \\
\bottomrule
\end{tabular}
\caption{ZAP Stack vs the reflection transaction codec (the codec
\texttt{avalanchego} master still runs), measured head-to-head on
Apple M1 Max, \texttt{go test -bench}, count$=5$ medians, identical
logical transactions (secp256k1, 2-in/2-out). The memory row is the
resident heap (\texttt{runtime.HeapAlloc} delta, GC-settled) a validator
holds for one live parsed block: ZAP keeps the buffer plus light views;
the codec materializes every field into the Go heap. The Corona row
