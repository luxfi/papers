...
\begin{remark}[Proof-level assumptions]
The bound above states the two \emph{standard} lattice assumptions for readability.
Ringtail's full proof~\cite{ringtail} reduces unforgeability to \emph{self-target}
Module-SIS (equivalent to Module-SIS in the ROM via the forking lemma) and to
\emph{Hint}-Module-LWE~\cite{hintmlwe}---a leakage variant that models the offline
commitment matrices---both of which reduce to standard Module-LWE and Module-SIS for the
parameters of Table~\ref{tab:params}. Corona inherits these reductions verbatim from the
Raccoon/Ringtail line; its permissionless distributed key generation and resharing affect
only key generation, not the per-signature unforgeability argument.
\end{remark}
