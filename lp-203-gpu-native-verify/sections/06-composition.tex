...
LP-218 & ZAP P3Q PQ-rollup. P3Q precompile at EVM slot
  \texttt{0x012205}; runs on the same Pulsar-verify GPU path
  this LP specifies \cite{lp218,hanzocrypto}. \\
\bottomrule
\end{tabular}
...
\item Stage 2: BLS pairing verify (GPU) -- the load-bearing
  measured kernel
\item Stage 3: Corona/Corona aggregate (GPU; once
...
LP-218 \cite{lp218,hanzocrypto} specifies a single EVM
precompile that verifies a Pulsar threshold signature against a
group public key. The precompile runs on whichever backend
\texttt{luxfi/accel} is built against -- on a Blackwell-class
validator, P3Q dispatches to the Pulsar GPU verify wrapper
...
SLH-DSA, Corona / Corona) are unchanged from their FIPS or
