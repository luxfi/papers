...
\title{\textbf{Trustless Interchain Token Transfers\\via Light Client Verification}\\[6pt]
...
\date{March 2026 \quad (Revised from February 2026)}
...
\subsection{MPC Threshold Security (March 2026 Revision)}
...
At $n = 100$ validators with $t = 67$ threshold and $p = 0.999$ individual uptime, the probability that the bridge cannot produce a valid signature is $\Pr[\text{unavailable}] < 2^{-139.6}$ (see \cite{lux-bridge-2026} Appendix~C for derivation).
...
The protocol is deployed for intra-Lux transfers and under development for Ethereum and Cosmos bridges, with mainnet launch targeted for Q2 2026.
...
\bibitem{lux-bridge-2026}
Z.~Kelling and W.~Bin, ``Lux Bridge: Threshold-secured cross-chain communication with MPC custody, post-quantum verification, and formally verified contract invariants,'' Lux Network, 2026.
