...
The ATS is implemented as a single Go binary (downstream-tenant ATS Go binary) running on Hanzo Base. Hanzo Base provides an embedded SQLite-compatible database with real-time subscriptions, authentication, and file storage. The binary packages all ATS functionality into a single deployable unit.
...
Integration & alpaca\_accounts, alpaca\_orders, simplici\_checks, provider\_configs \\
...
kyc\_poll & every 5m & Poll Simplici for KYC/AML status updates \\
